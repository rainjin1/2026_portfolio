#!/usr/bin/env python3
"""
qr_snapshot_node.py — FOV 기반 QR 스냅샷 탐색 노드 (cmd_vel 직접 제어)
========================================================================
동작 순서:
  1. /map 수신 → WallCoveragePlanner로 촬영 위치 목록 생성
  2. cmd_vel P컨트롤러로 각 촬영 위치에 이동 (Nav2 불필요)
  3. 도착 시 카메라에서 사진 1장만 캡처 → QR 감지
  4. QR 픽셀 위치 + 로봇 포즈 → QR 월드 좌표 역산
  5. 결과 저장 (YAML + TXT) + /qr/scan_complete 퍼블리시
...
class QRSnapshotNode(Node):
    ...
    def _scan_callback(self, msg: LaserScan):
        ...
        if n == 0:
            return
        # 전방 ±20° 인덱스 (LDS-01: 0°=전방, CCW)
        front_deg = 20
        front_idx = int(front_deg / 360.0 * n)
        indices = list(range(0, front_idx + 1)) + list(range(n - front_idx, n))
        valid = [msg.ranges[i] for i in indices
                 if 0.05 < msg.ranges[i] < float('inf') and not math.isnan(msg.ranges[i])]
        with self._scan_lock:
            self._obstacle_ahead = bool(valid) and min(valid) < OBSTACLE_DIST
    ...
    def _goto_pose(self, tx: float, ty: float, tyaw: float) -> bool:
        """
        목표(tx, ty, tyaw)까지 P컨트롤러로 이동.
        Phase1: 목표 방향으로 제자리 회전
        Phase2: 직선 전진 (조향 보정 포함)
        Phase3: 목표 yaw로 최종 정렬
        """
        start = time.time()
