...
MAX_WPC = 10  # WpC 빈 1개당 최대 수용량
MAX_BPC = 2   # BpC 빈 1개당 최대 수용량
MAX_CPC = 2   # CpC 빈 1개당 최대 수용량
...
def init_db():
    ...
    # 기존 DB 호환: color 컬럼 추가
    try:
        c.execute("ALTER TABLE inspection ADD COLUMN color TEXT")
    except Exception:
        pass
    ...
    # orders 테이블에 set_id 컬럼 추가 (기존 DB 호환)
    try:
        c.execute("ALTER TABLE orders ADD COLUMN set_id INTEGER REFERENCES order_sets(set_id)")
    except Exception:
        pass  # 이미 존재
...
def create_order_set() -> int:
    """주문 세트 생성. set_id 반환."""
    conn = _connect()
    cursor = conn.execute("INSERT INTO order_sets DEFAULT VALUES")
    conn.commit()
    set_id = cursor.lastrowid
    conn.close()
    print(f"[DB] 주문 세트 생성: #{set_id}")
    return set_id


def get_orders_by_set(set_id: int) -> list:
    """세트에 속한 주문 목록 반환."""
    conn = _connect()
    conn.row_factory = sqlite3.Row
    rows = conn.execute(
        "SELECT * FROM orders WHERE set_id=? ORDER BY order_id",
        (set_id,)
    ).fetchall()
    conn.close()
...
def create_order(pos_x, pos_y, pos_z,
    ...
    wall 색상: W/Y/P
    wall1=상, wall2=하, wall3=좌, wall4=우
    """
        ...
        cursor = conn.execute(
            """INSERT INTO orders
               (pos_x, pos_y, pos_z, base_color, ceil_color,
                wall1_color, wall2_color, wall3_color, wall4_color, set_id)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (pos_x, pos_y, pos_z, base_color, ceil_color,
             wall1, wall2, wall3, wall4, set_id)
        )
        conn.commit()
...
def clear_orders():
    """orders / process_log / inspection / order_sets 테이블 전체 삭제 (재고는 유지)."""
    conn = _connect()
    conn.execute("DELETE FROM orders")
    conn.execute("DELETE FROM process_log")
    conn.execute("DELETE FROM inspection")
    conn.execute("DELETE FROM order_sets")
    conn.commit()
    conn.close()
...
def add_inspection(order_id, face: int, result: str, color: str = None):
    """판별 결과 1면 기록. face=1~4, result='O'|'X', color=W/Y/P."""
    conn = _connect()
    conn.execute(
        "INSERT OR REPLACE INTO inspection (order_id, face, result, color) VALUES (?,?,?,?)",
        (order_id, face, result, color)
    )
    conn.commit()
    conn.close()
